...
Private Gallery Vault는 이미지, 영상, 문서, 압축 파일을 로컬 암호화 보관함에 정리하는 Windows 데스크톱 앱입니다. WPF와 .NET 기반으로 제작되었으며, 주제 폴더, 미디어 미리보기, 중복 검사, 빠른 잠금, 백업 도구를 제공합니다.

이 프로젝트는 로컬 우선 구조를 지향합니다. 파일은 보관함에 저장되기 전에 암호화되며, 보기나 외부 열기 과정에서 생성되는 임시 복호화 파일은 잠금 또는 종료 시 정리됩니다.
...
- 깨진 유니코드나 특수문자가 섞인 파일명 가져오기 안정화
...
GitHub에 올리기 전에 아래 파일은 포함하지 않는 것이 좋습니다.
